...
namespace StoreBuilder.Core.Net
{
    public class CloudflareEvader
    {
        ...
        public static HttpClient CreateBypassedWebClient(string url)
        {
            ...
            string agent = "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:40.0) Gecko/20100101 Firefox/40.0";
            ...
            if (!res.IsSuccessStatusCode)
            {
                ...
                var reuestMessage = new HttpRequestMessage(HttpMethod.Post, uri);
                reuestMessage.Headers.Add("Cookie", cookie);
                reuestMessage.Headers.Add("User-Agent", agent);
                var kjlk = client.SendAsync(reuestMessage).Result;
                ...
                var challenge = Regex.Match(html, "value=\"(\\w+)\" id=\"jschl-vc\"").Groups[1].Value;
                var challenge_pass = Regex.Match(html, "name=\"pass\" value=\"(.+?)\"").Groups[1].Value;
                var cpo = Regex.Match(html, "cpo.src=\"(.+?)\"").Groups[1].Value;

                var builder = Regex.Match(html, @"\(function\(\){\s+(var s,t,o,p,b,r,e,a,k,i,n,g,f.+?\r?\n[\s\S]+?a\.value =.+?)\r?\n").Groups[1].Value;
               ...
               var ljsl = JSEngine.Execute(url + cpo + "._cf_chl_opt.p=1").GetCompletionValue();
                //JSEngine.Execute();
                //    //Execute it. 
            ...
            return client;
            //}
            //catch (Exception ex) //We usually get this because of a 503 service not available.
            //{
            //    string html = "";
